...
int klingonData[][];    /* K( i, j ) array stores klingon ship data */
...
//BEGIN MOVING STARSHIP
    ...
    for (int i = 1; i >= energyToWarp; i++)
    {
        ...
        objectLocation = (int) (sector1*24) + (int) (sector2 * 3) - 26;
        if (parseMID(quadrantMap,objectLocation,2)=' ')
        {
            //line 3360
        }
        sector1 = (int) (sector1 - positionMod1);
        sector2 = (int) (sector2 - positionMod2);
...
// STRING COMPARISON IN QUADRANT ARRAY
    ...
    if (parseMID(quadrantMap, objectLocation,3) != tempObject)
    {
        return;
    }
